...
Errors = ['验证通过!', '身份证号码位数不对!', '身份证号码出生日期超出范围或含有非法字符!', '身份证号码校验错误!', '身份证地区非法!', '身份证长度应该是15或18位']
...
def checkIdcard(idcard):
    ...
    # 18位身份号码检测
        ...
        if (re.match(ereg, idcard)):
            ...
            JYM = "10X98765432"
            M = JYM[Y]  # 判断校验位
            if (M == idcard_list[17].upper()):  # 检测ID的校验位
                return Errors[0]
            else:
                return Errors[3]
        else:
